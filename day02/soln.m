...
input = textscan(fid,'%s %u');
fclose(fid);

input = [input{1} mat2cell(input{2},ones(length(input{2}),1))];
...
calc = @(key) sum([in{II == key,2}]);

horz = calc('forward')
depth = calc('down') - calc('up')
product = horz*depth
...
% units. It increases your depth by your aim multiplied by X. Again note
% that since you're on a submarine, down and up do the opposite of what you
...
% 2*10=20 to a total of 60. After following these new instructions, you
% would have a horizontal position of 15 and a depth of 60. (Multiplying
% these produces 900.)
% 
...
% Ok, now I lost the convenience of transitivity... :(
